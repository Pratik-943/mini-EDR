...
> **V1 lab topology:** this project uses exactly three systems on one trusted local network: one Ubuntu Server VM runs the central service, one Windows desktop VM is a monitored endpoint, and one Linux desktop VM is a monitored endpoint.
...
Only the Ubuntu Server runs the mini-EDR server, database, rule engine, dashboard, and HTTPS proxy. The Windows and Linux machines are endpoint systems; they only need to be able to reach the Ubuntu Server through the same local network.

| Resource | Minimum for this 3-system lab | Recommended |
|---|---:|---:|
...
| Open port | TCP `443` from both endpoints | TCP `443` from both endpoints |

The three systems must be connected to the same VMware Host-only or isolated NAT network. Give the Ubuntu Server a static IP address, because both endpoint agents use that address to connect.
...
This repository is published at `https://github.com/Pratik-943/mini-EDR`.
...
```powershell
git clone https://github.com/Pratik-943/mini-EDR.git C:\mini-edr
cd C:\mini-edr
python -m venv .venv
...
git clone https://github.com/Pratik-943/mini-EDR.git ~/mini-edr
cd ~/mini-edr
python3 -m venv .venv
. .venv/bin/activate
pip install -r requirements.txt
